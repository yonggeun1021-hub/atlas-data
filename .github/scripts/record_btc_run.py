#!/usr/bin/env python3
"""Persist one BTC price capture scheduler observation.

This operations-only record makes schedule/manual identity, runner delay,
capture/skip/failure, and validation outcome observable from a read-only clone.
It never calls Kraken, reads the captured market payload, evaluates a signal, or
grants Production/trading authority.
"""
...
OUT_ROOT = ROOT / "data" / "operations" / "btc_capture_runs"
UTC = dt.timezone.utc
KST = dt.timezone(dt.timedelta(hours=9))

SCHEDULE_SLOTS = {
    "20 0 * * *": ("daily_0920_kst", 0, 20),
}


class TelemetryError(RuntimeError):
    """BTC operations telemetry contract violation."""
...
def validation_observation(step_outcome: str) -> dict:
    ...
    return {
        "step_outcome": outcome or "unknown",
        "result": result,
        "raw_publication_eligible": result == "passed",
    }
...
def build_record(environ: dict[str, str]) -> dict:
    ...
    return {
        "schema_version": 1,
        "workflow": "BTC Price Daily Capture",
        ...
        "validation": validation_observation(
            environ.get("ATLAS_VALIDATION_STEP_OUTCOME", "")
        ),
        "authority_flags": {
...
def run(argv=None, environ=None) -> int:
    ...
    print(
        ...
        f" validation={record['validation']['result']}"
        f" path={target}"
    )
    return 0
