#!/usr/bin/env python3
"""Persist one P3-12 Upbit universe capture workflow scheduler observation.

The record is operations telemetry only. It allows a read-only clone to
distinguish schedule/manual execution and capture/classification outcomes
...
OUT_ROOT = ROOT / "data" / "operations" / "upbit_universe_capture_runs"
UTC = dt.timezone.utc
KST = dt.timezone(dt.timedelta(hours=9))

SCHEDULE_SLOTS = {
    "50 0 * * *": ("daily_0950_kst", 0, 50),
}


class TelemetryError(RuntimeError):
    """P3-12 operations telemetry contract violation."""
...
def classification_observation(step_outcome: str, result: str, reason: str, path: str, sha256: str) -> dict:
    outcome = step_outcome.strip().lower() if isinstance(step_outcome, str) else ""
    declared = result.strip().lower() if isinstance(result, str) else ""
    if outcome == "failure":
        normalized = "failed"
...
def build_record(environ: dict[str, str]) -> dict:
    ...
    return {
        "schema_version": 1,
        "workflow": "P3-12 Upbit KRW Tradeable Universe Daily Capture",
        ...
        "p3_12_classification": classification_observation(
            environ.get("ATLAS_P3_12_STEP_OUTCOME", ""), environ.get("ATLAS_P3_12_RESULT", ""),
            environ.get("ATLAS_P3_12_REASON", ""), environ.get("ATLAS_P3_12_PATH", ""),
            environ.get("ATLAS_P3_12_SHA256", ""),
        ),
        "authority_flags": {
...
def run(argv=None, environ=None) -> int:
    ...
    print(
        "Upbit universe scheduler telemetry"
        ...
        f" p3_12={record['p3_12_classification']['result']}"
        f" path={target}"
    )
    return 0
