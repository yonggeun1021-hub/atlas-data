...
detection, run over real committed evidence) and `clock/dynamic_clock.py`
(the new episode/cooldown/expiry/reactivation state machine) into one
deterministic report, split by market (BTC/KOREA/CRYPTO -- never blended,
same discipline as PR #210's `by_market`).
...
★ Authority: this module never sets Stage/Buy/Action/Order/Production/
  trading. Every `ReviewCandidate.authority` block is hard-`False`/`None`
  (see `review_candidate.py`). A Trigger firing here is a re-review REQUEST
  only.
"""
...
from clock.review_candidate import build_expired_record, build_review_candidate
...
def _market_result(market: str) -> dict:
    ...
    active_candidates.sort(key=lambda r: (r["subject"], r["trigger_type"], r["candidate_id"]))
...
def run() -> dict:
    ...
    return {
        ...
        "authority_note": (
            ...
            "never confers Action/Order/trading authority. See each record's own `authority` block."
        ),
        "by_market": by_market,
    }
...
def build_briefing_section(report: dict) -> dict:
    """The additive, standalone artifact item 6 asks for: 'new triggers,
    expired triggers, and tickers needing re-review' as a section a briefing
    module COULD read. Deliberately NOT wired into
    `briefing/daily_orchestrator.py` in this PR -- that file is actively
    owned/touched by the concurrent Forward Alpha session; see the PR
    description's conflict-check section. A future, separate PR can import
    ...
    for market, m in report["by_market"].items():
        section["markets"][market] = {
            ...
            "needs_re_review": [
                ...
                for r in m["active_review_candidates"]
            ],
            "expired_triggers": [
