...
| 8 | `PAPER_RISK_BUDGET` | Mechanical, against a caller-supplied virtual PAPER account snapshot. Enforces all of ratified build plan row C4 / `RULE.ALLOCATION.V2` together. **Aggregate cap** = `NAV0 x base(CRYPTO) x per_market_state_multiplier`, resolved from the rule registry (never a flat fraction in this repo's config — the field was removed). The crypto market state is read point-in-time from the `CRYPTO_PAPER_RUNTIME_V1` decision the contract/3 promotion packet already carries, at the same reference instant the promotion packet used; it is never computed here and never defaults to RISK_ON. `STRESS` → zero cap; `RISK_OFF`/`STRESS`/`UNKNOWN` → `new_buys: DENY` and a hard breach; a missing, non-current, or stale-beyond-`maximum_observation_gap_days` (CRYPTO = 2) state → `UNKNOWN`, which carries both the ratified `0.50`-of-base hold cap **and** the hard no-new-buys flag. **Per-name cap** = `min(NAV x single_asset_paper_exposure_nav_fraction, 1% x ADV30)`, where ADV30 is derived point-in-time from the already-validated P3-12 universe row (`trailing_30d_krw_turnover` over its finalized day count, window from `turnover_lookback_finalized_days`). A market with fewer than the full ratified window of committed finalized days **fails closed** — a shorter window is never substituted. Both cap terms, the effective cap, and which term bound (`per_name_cap_bound_by`) are recorded on PASS and FAIL alike. Because the per-name cap is a NAV fraction ≤ 5% while the aggregate cap is `base x multiplier`, the per-name cap can never equal the aggregate cap under any ratified state that permits new buys — so one coin can never consume the whole crypto budget. The old fixed 3-concurrent-position cap is superseded per ratification and no longer enforced (the count is kept for observability). Existing total exposure is the sum of `portfolio_weight_nav_fraction`, not planned-loss fractions. UNKNOWN when no snapshot is supplied — no NAV is known. |
