...
| 8 | `PAPER_RISK_BUDGET` | Mechanical, against a caller-supplied virtual PAPER account snapshot. Checks projected total/single-asset PAPER exposure against a 5% aggregate / 5% single-asset cap. The single-asset cap (5%) is ratified (build plan row C4, 2026-09-18); the aggregate cap is left at the old, more conservative 5% flat value because the ratified rule (`NAV0 x 0.15 x` the crypto market's own state multiplier) needs a crypto-regime-state input this module does not receive -- see the config's `source_reference` and the comment above `_paper_risk()`. The old fixed 3-concurrent-position cap is superseded per ratification and no longer enforced; a ratified 1%-of-30-day-ADV per-name liquidity cap is also not enforced (no ADV input reaches this function). Existing total exposure is the sum of `portfolio_weight_nav_fraction`, not planned-loss fractions. UNKNOWN when no snapshot is supplied — no NAV is known. |
