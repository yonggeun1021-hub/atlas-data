...
def _validate_dynamic_clock_frozen_source(
    ...
    """Bind a present P8-12 source without rejecting legacy Daily packets."""
    ...
    if type(source) is not dict:  # noqa: E721 - exact JSON boundary
        raise FlowFirstBriefingError("SOURCE_DYNAMIC_CLOCK_INVALID:source_type")
    kind = source.get("kind")
    if type(kind) is not str:  # noqa: E721 - reject bool/string aliases
        raise FlowFirstBriefingError("SOURCE_DYNAMIC_CLOCK_INVALID:kind_type")
    if kind == "unavailable":
        if set(source) != {"kind"}:
            raise FlowFirstBriefingError(
    ...
    if kind != "report" or set(source) != {"kind", "report_sha256", "report"}:
        raise FlowFirstBriefingError("SOURCE_DYNAMIC_CLOCK_INVALID:report_shape")
    ...
    if payload_sha256(report) != report_sha256:
        raise FlowFirstBriefingError("SOURCE_DYNAMIC_CLOCK_SHA_MISMATCH")
    if report.get("decision_date") != decision_date.isoformat():
        raise FlowFirstBriefingError("SOURCE_DYNAMIC_CLOCK_DATE_MISMATCH")
