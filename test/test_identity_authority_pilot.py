...
class IdentityAuthorityPilotTests(unittest.TestCase):
    ...
    def test_korea_claims_match_krx_dart_and_identity_contract(self):
        ...
        master = json.loads(
            (ROOT / "data/observations/krx_global_universe/2026-08-27/packet.json").read_text()
        )["asset_master"]["records"]
        korea_investment = next(row for row in master if row["asset_id"] == "KR:XKRX:071050")
        self.assertEqual(korea_investment["display_name"], "한국금융지주")
        self.assertEqual(korea_investment["primary_symbol"], "071050")
        self.assertEqual(corp_map["071050"], "00432102")
        self.assertFalse(korea_investment["investable_eligible"])
