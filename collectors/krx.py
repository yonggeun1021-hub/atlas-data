...
v2 (2026-08-13) — 수정 2건
  ① 외국인 컬럼 누락 수정
     pykrx 기본 조회(detail=False)의 컬럼명은 '외국인합계'이지 '외국인'이 아니다.
     v1은 '외국인'을 찾다가 조용히 건너뛰어 외국인 수급이 통째로 빠졌다.
  ② 조용한 누락 금지
     요청한 투자자 구분이 응답에 없으면 건너뛰지 않고
     missing_investors 에 명시한다. 빈칸을 빈칸으로 남긴다.
"""
import os
import sys
...
def main() -> None:
    ...
    for s in load_universe():
        ...
        try:
            ...
            print(f"[ok]     {code} {name}" + (f"  ⚠ 누락: {miss}" if miss else ""))
